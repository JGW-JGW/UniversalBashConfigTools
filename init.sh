...
HOSTNAME=$(hostname)
MANUFACTURER=$(init_strip "$([[ -f /dev/mem ]] && dmidecode -t1 | grep "Manufacturer:" | awk -F: '{print $2}')")
PRODUCT_NAME=$(init_strip "$([[ -f /dev/mem ]] && dmidecode -t1 | grep "Product Name:" | awk -F: '{print $2}')")
